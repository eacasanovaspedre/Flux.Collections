...
let tests =
    ftestList "Stream<_>"
        [ testCase "empty should create an empty lazy list" <| fun () ->
            match empty with
            | Cons _ -> fail "Stream was not empty"
            | Nil -> pass

          testProperty "singleton should create a lazy list with exactly the original item" <| fun x ->
              match singleton x with
              | Cons(x', Nil) -> x = x'
              | Cons(_, Cons _) -> failProp
              | Nil -> failProp

          testProperty "cons should add the item to the lazy list as the new head" <| fun (Finite l) x ->
              match cons x l with
              | Cons(x', l') -> x' = x && l' = l
              | _ -> fail ""

          testProperty "consDelayed should add the item to the lazy list as the new head" <| fun f x ->
              let ff() =
                  match f() with
                  | Finite l -> l
          ...
          testProperty "isEmpty returns whether the lazy list contains no elements" <| fun laz ->
              laz
              |> match laz with
                 | Cons _ -> isEmpty >> not
                 | Nil -> isEmpty

          testProperty "isNotEmpty returns whether the lazy list contains elements" <| fun laz ->
              laz
              |> match laz with
                 | Cons _ -> isNotEmpty
                 | Nil -> isNotEmpty >> not

          testProperty "head should return the first element of the lazy list" <| fun laz ->
              isNotEmpty laz ==> match laz with
                                 | Cons(h, _) -> head laz = h
                                 | _ -> passProp

          testCase "head should throw when the lazy list is empty"
          <| fun () ->
          ...
          testProperty
              "maybeHead should return 'Some' with the first element of the lazy list or 'None' if the list is empty" <| fun laz ->
              match laz, maybeHead laz with
              | Cons(h, _), Some h' -> h' = h
              | Nil, None -> passProp
              | _ -> failProp

          testCase "tail should throw when the lazy list is empty"
          <| fun () ->
              Expect.throwsT<System.InvalidOperationException>
          ...
          testProperty
              "maybeTail should return 'Some' with the lazy list with the first element removed or 'None' if the list is empty" <| fun (Finite laz) ->
          ...
          testProperty
              "uncons should return the head and tail pair if the list is not empty and it should throw if the lazy list is empty" <| fun (Finite laz) ->
              match laz with
              | Cons p ->
          ...
          testProperty
              "maybeUncons should return 'Some' with the head-tail pair if the list is not empty or 'None' if the list is empty" <| fun (Finite laz) ->
          ...
          testProperty
              "last should return the last item if the list is not empty or it should throw if the list is empty" <| fun (Finite laz) ->
              if isEmpty laz then
          ...
          testProperty
              "maybeLast should return the last item wrapped in 'Some' if the list is not empty or it should return 'None' otherwise" <| fun (Finite laz) ->
              laz
              |> asList
              |> List.tryLast = maybeLast laz

          testProperty "count should return the number of items in the list"
          <| fun (Finite laz) -> Seq.length laz = count laz

          testProperty "length should return the number of items in the list"
          <| fun (Finite laz) -> Seq.length laz = length laz

          testProperty "init creates a lazy list with the number of items specified using the item creator"
          ...
          testProperty
              "initInfinite creates an infinite lazy list using the item supplier. The first m items are correct"
          ...
          testProperty
              "initWhile creates a lazy list with elements supplied by the function as long as its result is 'Some'" <| fun (NonNegativeInt n) f ->
              let f' =
                  function
                  | i when i = n -> None
                  | i -> Some(f i)
          ...
          testProperty "replicate creates a lazy list with the supplied item repeated the number of times specified"
          ...
          testProperty
              "append creates a lazy list that contains the elements of the first list followed by the elements of the second list"
          ...
          testProperty "concat creates a lazy list that contains the elements of each list in the same other" <| fun (NonNegativeInt n) f ->
          ...
          testProperty "concat works with infinite lists" <| fun (PositiveInt n) f ->
              let list = init n (f >> infinite)
              infiniteSequenceEqual (concat list) (f 0 |> infinite)

          testProperty "concat works with infinite list of lists" <| fun f ->
              let list = initInfinite f
              infiniteSequenceEqual (concat list) (Seq.initInfinite f |> Seq.concat)

          testProperty "concat does not evaluate the original list of lists"
          <| fun z -> isSuspended z ==> (concat z |> isSuspended)

          testProperty
              "collect creates a lazy list with the concatenation of the lazy lists resulted from mapping each element of the original list to a lazy list" <| fun (Finite z) f ->
              collect (f >> finite) z
              |> asList =
                  (asList z
                   |> List.collect
          ...
          testProperty
              "collecti creates a lazy list with the concatenation of the lazy lists resulted from mapping each element of the original list to a lazy list" <| fun (Finite z) f ->
          ...
          testProperty "collecti works with infinite lists"
          <| fun z f ->
              infiniteSequenceEqual (collecti (fun i -> f i >> infinite) z)
                  (z |> Seq.collecti (fun i -> f i >> infinite))

          testProperty "collecti does not evaluate the original list"
          ...
          testProperty "scan scans infinite lists"
          <| fun (Infinite z) f s -> infiniteSequenceEqual (scan f s z) (Seq.scan f s z)

          testProperty "scan returns an unevaluated list" <| fun z f s -> scan f s z |> isSuspended

          testProperty "scan does not evaluate the original list"
          ...
          testProperty "scanBack does not evaluate the original list"
          ...
          testProperty "reduce throws when the list is empty"
          ...
          testProperty "reduceBack throws when the list is empty"
          ...
          testProperty "choose chooses in a infinite list" <| fun f (Infinite z) -> infiniteSequenceEqual (choose f z) (Seq.choose f z)

          testProperty "choose returns an unevaluated list" <| fun f z -> choose f z |> isSuspended

          testProperty "choose does not evaluate the original list"
          ...
          testProperty "filter filters an infinite list"
          <| fun f (Infinite z) -> infiniteSequenceEqual (filter f z) (Seq.filter f z)

          testProperty "filter returns an unevaluated list" <| fun f z -> filter f z |> isSuspended

          testProperty "filter does not evaluate the original list"
          ...
          testProperty "map works with infinite lists"
          <| fun f (Infinite z) -> infiniteSequenceEqual (map f z) (Seq.map f z)

          testProperty "map returns a suspended list" <| fun f z -> map f z |> isSuspended
          ...
          testProperty "mapi works with infinite lists"
          <| fun f (Infinite z) -> infiniteSequenceEqual (mapi f z) (Seq.mapi f z)

          testProperty "mapi returns a suspended list" <| fun f z -> mapi f z |> isSuspended

          testProperty "mapi does not evaluate the original list"
          ...
          testProperty "map2 works with infinite lists"
          ...
          testProperty "map2 does not evaluate the original list"
          ...
          testProperty "zip works with infinite lists"
          <| fun f (Infinite z1) (Infinite z2) -> infiniteSequenceEqual (zip z1 z2) (Seq.zip z1 z2)

          testProperty "zip returns a suspended list" <| fun f z1 z2 -> zip z1 z2 |> isSuspended

          testProperty "zip does not evaluate the original list"
          ...
          testProperty "zip3 works with infinite lists"
          <| fun f (Infinite z1) (Infinite z2) (Finite z3) ->
              infiniteSequenceEqual (zip3 z1 z2 z3) (Seq.zip3 z1 z2 z3)

          testProperty "zip3 returns a suspended list" <| fun f z1 z2 z3 -> zip3 z1 z2 z3 |> isSuspended

          testProperty "zip3 does not evaluate the original list"
          <| fun f z1 z2 z3 ->
          ...
          testProperty "take throws when asked to take more elements than exists in the list"
          <| fun (Sized(z, size)) n ->
          ...
          testProperty "drop is consistent with List's skip when there are enough elements in the list to be dropped"
          <| fun (Sized(z, size)) (NonNegativeInt n) ->
              n <= size ==> lazy (drop n z
                                  |> asList = List.skip n (asList z))

          testProperty "drop returns empty when asked to drop more elements than exists in the list"
          ...
          testProperty "skip is consistent with List's skip when there are enough elements in the list to be skipped"
          <| fun (Sized(z, size)) (NonNegativeInt n) ->
              n <= size ==> lazy (skip n z
                                  |> asList = List.skip n (asList z))

          testProperty "skip throws when asked to skip more elements than exists in the list"
          <| fun (Sized(z, size)) (NonNegativeInt n) ->
          ...
          testProperty "maybeSkip returns the same list when asked to skip a negative or zero number of elements"
          <| fun (Finite z) (PositiveInt n) -> maybeSkip -n z = Some z

          testProperty "maybeSkip is consistent with skip when there are enough elements in the list"
          <| fun (Sized(z, size)) (NonNegativeInt n) -> n <= size ==> lazy (maybeSkip n z = Some(skip n z))

          testProperty "maybeSkip returns None when asked to skip more elements that exists in the list"
          ...
          testProperty "skipWhile only evaluates the skipped elements and the first element of the remaining list" <| fun (Infinite z) f ->
          ...
          testList "Laziness preservation"
              [ testProperty "cons should not evaluate the list" <| fun l x ->
                  isSuspended l ==> (l
                                     |> cons x
                                     |> tail
                                     |> isSuspended)

                testProperty "consDelayed should not evaluate the list" <| fun f x ->
                    isSuspended (f()) ==> (f
                                           |> consDelayed x
                                           |> tail
                                           |> isSuspended)

                testProperty "uncons should not evaluate the tail of the list" <| fun l x ->
                    isSuspended l ==> (l
                                       |> cons x
                                       |> uncons
                                       |> snd
                ...
                testProperty "tryUncons should not evaluate the tail of the list" <| fun l x ->
                    isSuspended l ==> (l
                                       |> cons x
                                       |> maybeUncons
                                       |> Option.get
